...
class FanWiki {
    ...
    async simpleSearch(query) {
        try {
        const response = await axios.get(`https%3A%2F%2Fservices.fandom.com%2Fmobile-fandom-app%2Funified-search%2Fsummary%3Fquery%3D${encodeURIComponent(query)}%26language%3Din`)
        return {
            status: 200,
